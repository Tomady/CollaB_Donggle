...
document.querySelector("#starredSearch").onclick = function(){
	...
	if(searchVal == ""){ //검색값이 없을때
		...
		document.querySelector("#starredSearchVal").focus();
	}else{
		if(jogun == "writer"){
			$.ajax({
				url : "AjaxStarredSearch_writer",
				type : "POST",
				data : {
					searchVal : searchVal
				},
				...
				success : function(datas){
					//검색창 지워주고
					document.querySelector("#starredSearchVal").value = "";
...
function show_joinMembers(wkid){
	...
	members.innerHTML = "";
	let id = document.querySelector("#starredTitle").getAttribute("data-id");
...
//delete 버튼 클릭시
document.getElementById("starredMsgDelete").onclick=function(){
