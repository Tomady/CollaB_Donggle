...
function deleteBoard(id){
	//삭제될 보드란에 아무것도 없도록 내용물 비우기
	deleteTargetBoard.innerHTML = "";

	var zIndex = 9999;
    var modal = document.getElementById(id);
	
    // 닫기 버튼 처리, 시꺼먼 레이어와 모달 div 지우기
    modal.querySelector('.del_board_close_btn').onclick=function(){ closeDelBoard() };
	
	// 보드 삭제 버튼 처리
	modal.querySelector('#proceedBoardDelete').onclick=function(){
		//삭제하려고 선택한 보드목록
		const delTargetBoards = document.querySelectorAll(".delTargetBoard");
		//삭제하려고 선택한 보드아이디들을 배열에 담아주기
		const boardIds = new Array();
		for(let i = 0 ; i<delTargetBoards.length; i++){
			boardIds[i] = delTargetBoards[i].getAttribute("data-boardid");
		}
		console.log("삭제하고자하는 보드아이디들",boardIds);
		//아작스로 삭제해준다음에 삭제완료되면 closeDelBoard() 해서 모달창 닫아주고
		//window.location.onload()로 새로고침한번 해주기
	}
	
	function closeDelBoard(){
		bg.remove();
...
//보드삭제_보드하나 클릭했을때
function del_SelectedBoard(boardid,boardTitle){
	
	let btn = document.createElement("button");
	btn.setAttribute("class","delTargetBoard btn btn-light mt-2 mb-2 ml-2");
	btn.setAttribute("data-boardid",boardid);
	btn.style.width="95%";
	btn.innerHTML = boardTitle;
	
	deleteTargetBoard.prepend(btn);
}
