							...
							<div class="card-body">
								<div class="card-header d-flex justify-content-center"
									...
									<div id="workspace-body"
										...
										<div class="row workspaceNAME">
											<img class="img-fluid mr-2 wkimg-body">
								...
								<div class="card-body ml-5 mr-5" style="height: 75vh;">
									<div class="float-left">
										<a
											href="goIssueInsert.do?workspace_id=${workspace.workspace_id}"
											class="btn btn-icon icon-left btn-primary"
											style="background-color: #6553C1; box-shadow: 0 2px 6px #ECE9FE;"><i
											class="far fa-edit"></i> 이슈등록</a>
									</div>
											...
											<tbody>
												<c:forEach items="${issues }" var="issue">
													<tr>
														...
														<td id="gogo"
															onclick="location.href='issueDetail.do?issueId=${issue.issueId}&workspace_id=${workspace.workspace_id}'">${issue.issueTitle }</td>
														<td>${issue.issueCategory }</td>
														<td>
															<div class="progress mb-3">
																<div class="progress-bar" role="progressbar"
																	aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"
																	style="background-color: #6553C1"></div>
															</div>
														</td>
													</tr>
												</c:forEach>
	...
	<script>
		...
		function page(pagenumber, pageCount, currentPage) {
			...
			for (let i = first; i <= last; i++) {
				$('<a href="#"></a>').attr('data', i).html(i).appendTo(
						'#pagebtn');
			}
			if (last < pagetotal) {
				...
				.ready(
						function() {
							const badge = document
									.getElementsByClassName('badge badge-success');

							for (let i = 0; i < badge.length; i++) {
								if (badge[i].innerText === 'TO DO') {
									badge[i].setAttribute('class',
											'badge badge-danger');
								}
							}
